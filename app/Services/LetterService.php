...
final class LetterService {
    ...
    public static function html(array $d, bool $pdf = false): string {
        ...
        $lineHtml = match($v['kop_line_style'] ?? 'double') {
            'single_thick' => '<div style="border-top:2.5px solid #000;margin:6px 0 16px;"></div>',
            'single_thin' => '<div style="border-top:1px solid #000;margin:6px 0 16px;"></div>',
            'dashed' => '<div style="border-top:1.5px dashed #000;margin:6px 0 16px;"></div>',
            'none' => '<div style="margin:12px 0;"></div>',
            default => '<div style="border-top:3px solid #000;border-bottom:1px solid #000;height:2px;margin:6px 0 16px;"></div>'
        };

        $align = ($v['kop_align'] ?? 'center') === 'left' ? 'left' : 'center';

        // KOP Header Table
        $html = '<div style="' . $fontStyle . 'line-height:1.45;color:#111;font-size:12pt;">';
        ...
        $html .= '<div style="font-size:14pt;font-weight:bold;text-transform:uppercase;line-height:1.2;">' . $e($v['institution']) . '</div>';
        if (!empty($v['unit'])) {
            $html .= '<div style="font-size:12pt;font-weight:bold;text-transform:uppercase;line-height:1.25;margin-top:2px;">' . $e($v['unit']) . '</div>';
        }
        $html .= '<div style="font-size:9pt;margin-top:3px;line-height:1.3;">' . $lines($v['address']) . '</div>';
        if (!empty($v['contact'])) {
            $html .= '<div style="font-size:8.5pt;margin-top:2px;">' . $e($v['contact']) . '</div>';
        }
        $html .= '</td>';
        ...
        $html .= '<table width="100%" cellpadding="2" style="border-collapse:collapse;margin-bottom:14px;font-size:11pt;"><tr>';
        ...
        $html .= '<div>' . $dateStr . '</div><br>';
        $html .= '<div style="text-align:left;display:inline-block;">Kepada Yth.<br>' . $lines($v['recipient']) . '</div>';
        $html .= '</td>';
        $html .= '</tr></table>';

        // Salam Pembuka & Paragraf Pembuka
        if (!empty($v['greeting_opening'])) {
            $html .= '<p style="margin:12px 0 6px 0;">' . $e($v['greeting_opening']) . '</p>';
        }
        if (!empty($v['opening'])) {
            $html .= '<p style="text-align:justify;line-height:1.6;margin:0 0 10px 0;">' . $lines($v['opening']) . '</p>';
        }

        // Isi Dokumen
        if (!empty($v['body'])) {
            $html .= '<div style="text-align:justify;line-height:1.6;margin:0 0 12px 0;">' . $lines($v['body']) . '</div>';
        }

        // Paragraf Penutup & Salam Penutup
        if (!empty($v['closing'])) {
            $html .= '<p style="text-align:justify;line-height:1.6;margin:0 0 10px 0;">' . $lines($v['closing']) . '</p>';
        }
        if (!empty($v['greeting_closing'])) {
            $html .= '<p style="margin:8px 0 16px 0;">' . $e($v['greeting_closing']) . '</p>';
        }

        // Tanda Tangan & Cap Stempel
        $html .= '<table width="100%" cellpadding="2" style="border-collapse:collapse;margin-top:20px;"><tr>';
        $html .= '<td width="55%"></td>';
        $html .= '<td width="45%" align="center" style="font-size:11.5pt;">';
        $html .= '<div>' . $e($v['signer_position']) . '</div>';
        
        $html .= '<div style="position:relative;min-height:75px;margin:8px 0;display:flex;align-items:center;justify-content:center;">';
        if ($signature) {
            $html .= '<img src="' . $signature . '" style="max-height:70px;max-width:140px;object-fit:contain;vertical-align:middle;">';
        } else {
        ...
        if ($stamp) {
            ...
            $html .= ' <img src="' . $stamp . '" style="position:absolute;' . $stampPosStyle . 'max-width:85px;max-height:85px;opacity:' . $opacity . ';vertical-align:middle;">';
        }
        $html .= '</div>';

        $html .= '<div style="font-weight:bold;text-decoration:underline;margin-top:4px;">' . $e($v['signer_name']) . '</div>';
        if (!empty($v['signer_number'])) {
            $html .= '<div style="font-size:10pt;">' . $e($v['signer_number']) . '</div>';
        }
        $html .= '</td></tr></table>';

        // Tembusan
        if (!empty($v['copies'])) {
            $html .= '<div style="margin-top:24px;font-size:10pt;line-height:1.4;">';
        ...
        if (!empty($v['footer_note'])) {
            $html .= '<div style="margin-top:30px;padding-top:6px;border-top:0.5px solid #ccc;font-size:8.5pt;color:#666;font-style:italic;">';
